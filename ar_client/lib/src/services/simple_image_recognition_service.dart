...
class SimpleImageRecognitionService {
  ...
  String computeHash(Uint8List imageBytes) {
    try {
      ...
      print('   🔄 Resizing to ${hashSize}x$hashSize...');
      ...
      print('   📊 Average brightness: ${average.toStringAsFixed(1)}');
      ...
      print('   🔑 Hash generated: $hash (${hash.length} chars)');

      return hash;
    } catch (e) {
      print('   ❌ Hash computation error: $e');
  ...
  int hammingDistance(String hash1, String hash2) {
    if (hash1.length != hash2.length) {
      print('   ⚠️  Hash length mismatch: ${hash1.length} vs ${hash2.length}');
  ...
  bool areSimilar(String hash1, String hash2, {int threshold = 15}) {
    ...
    print('   📏 Distance: $distance bits (threshold: $threshold)');
